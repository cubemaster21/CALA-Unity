						...
						for (int i = 2; i < n.Length; i++) {
							...
							else if (ch == '-' || ch == ':')
							{
								if (isLastRange)
									break;
								isLastRange = true;
								rangeStart = isChar ? (int)n[i - 1] : int.Parse(n.Substring(startPos, i - startPos), isHex ? NumberStyles.AllowHexSpecifier : NumberStyles.None);							
								startPos = i + 1;
							} else if (ch == ',' || ch == '&' || ch == ';' || i == last)
							{
								if (i == last && !(ch == ',' || ch == '&' || ch == ';' ))
									i++;
								var parsed = isChar ? (int)n[i - 1] : int.Parse(n.Substring(startPos, i - startPos), isHex ? NumberStyles.AllowHexSpecifier : NumberStyles.None);
								if (isLastRange) {
