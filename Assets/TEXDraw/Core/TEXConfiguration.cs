...
namespace TexDrawLib
{
    ...
    {
        static TEXConfiguration m_main;

        public static TEXConfiguration main
        {
            get {
                if (!m_main) {
                    // The only thing that we can found is in the Resource folder
                    if (TEXPreference.main)
                        m_main = TEXPreference.main.preferences;
                    if (!m_main)
                    {
                        m_main = TEXPreference.main.preferences = (TEXConfiguration)Resources.Load("TEXDrawConfiguration");
#if UNITY_EDITOR
                        if (!m_main)
                        {
                            m_main = TEXPreference.main.preferences = ScriptableObject.CreateInstance<TEXConfiguration>();
                            UnityEditor.AssetDatabase.CreateAsset(m_main, TEXPreference.main.MainFolderPath + "/Resources/TEXDrawConfiguration.asset");
                        }
        ...
        [Range(0, 31)]  public int Typeface_Number;
        [Range(0, 31)]  public int Typeface_Capitals;
        [Range(0, 31)]  public int Typeface_Small;
        [Range(0, 31)]  public int Typeface_Commands;
        [Range(0, 31)]  public int Typeface_Text;
        [Range(0, 31)]  public int Typeface_Unicode;
